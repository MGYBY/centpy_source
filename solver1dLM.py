...
def source_mom_implicit(p, h0, q0, up0, delta_t, alpha, beta):
    # x stands for q^(n+1), y stands for up^(n+1)
    x, y = p
    return (x-q0-delta_t/beta*(h0-alpha-(2.0*y)/(3.0*(h0-x/y))), y-up0-delta_t/beta*(1.0-alpha/(-2.0*h0+3.0*x/y)))

# the equation for first-step implicit RK
def source_ir_1(p, h, q, up, delta_t, alpha, beta, a11):
    x, y = p
    # ho = 3.0*(h-q/up)
    return (x-1.0/beta*(h-alpha*(1.0)-2.0*(up+delta_t*a11*y)/(3.0*(h-(q+delta_t*a11*x)/(up+delta_t*a11*y)))), y-1.0/beta*(1.0-alpha*(1.0)/(h-(3.0*(h-(q+delta_t*a11*x)/(up+delta_t*a11*y))))))

# the equation for second-step implicit RK
def source_ir_2(p, h, q, up, p01, p02, delta_t, alpha, beta, a21, a22):
    x, y = p
    # ho = 3.0*(h-q/up)
    return (x-1.0/beta*(h-alpha*(1.0)-2.0*(up+delta_t*(a21*p02+a22*y))/(3.0*(h-(q+delta_t*(a21*p01+a22*x))/(up+delta_t*(a21*p02+a22*y))))), y-1.0/beta*(1.0-alpha*(1.0)/(h-(3.0*(h-(q+delta_t*(a21*p01+a22*x))/(up+delta_t*(a21*p02+a22*y)))))))
...
class Solver1dLM:
    ...
    def source_backward_euler(self, u, t):
        ...
        for i in range(2, arr_length-1):
            u[i,1], u[i,2] = optimize.fsolve(source_mom_implicit, (u[i,1], u[i,2]), args=(u[i,0], u[i,1], u[i,2], self.dt, alpha_coeff, beta_coeff))
    ...
    def source_crank_nicolson(self, u, t):
        ...
        # use the iterative solver in scipy to solve the nonlinear equation set
        for i in range(2, arr_length-1):
            q_implicit, up_implicit = optimize.fsolve(source_mom_implicit, (u[i,1], u[i,2]), args=(u[i,0], u[i,1], u[i,2], self.dt, alpha_coeff, beta_coeff))
            q_explicit = u[i,1]+self.dt*source_mom_1(u[i,0], u[i,1], u[i,2], alpha_coeff, beta_coeff)
            up_explicit = u[i,2]+self.dt*source_mom_2(u[i,0], u[i,1], u[i,2], alpha_coeff, beta_coeff)
            u[i,1] = (q_explicit+q_implicit)/2.0
            u[i,2] = (up_explicit+up_implicit)/2.0
