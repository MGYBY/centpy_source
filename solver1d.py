...
class Solver1d:
    ...
    ###################################################
    def source_int(self, u):
        u_int = np.ones(u.shape)
        # first stage
        u_int[j0] = u[j0] + self.dt*(self.source(u[j0]))
        self.boundary_conditions(u_int)
        # second stage
        u_int[j0] = (3.0/4.0)*u[j0] + (1.0/4.0)*u_int[j0] + (1.0/4.0)*self.dt*(self.source(u_int[j0]))
        self.boundary_conditions(u_int)
        # third stage
        u[j0] = (1.0/3.0)*u[j0] + (2.0/3.0)*u_int[j0] + (2.0/3.0)*self.dt*(self.source(u_int[j0]))
        self.boundary_conditions(u)
        return u
