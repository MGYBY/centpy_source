...
def source_mom(h, q, cf, slope, g):
        return (g*slope*h-cf/2.0*((q/h)**2))
...
class Solver1dSWE:
    ...
    def source_int(self, u, t):
        ...
        for i in range(2, arr_length-1):
            # first stage
            u_int = u[i,1] + self.dt*(source_mom(u[i,0], u[i,1], fc, channel_slope, gravity_coeff))
            # second stage
            u_int = (3.0/4.0)*u[i,1] + (1.0/4.0)*u_int + (1.0/4.0)*self.dt*(source_mom(u[i,0], u_int, fc, channel_slope, gravity_coeff))
            # self.boundary_conditions(u_int, t)
            # third stage
            u[i,1] = (1.0/3.0)*u[i,1] + (2.0/3.0)*u_int + (2.0/3.0)*self.dt*(source_mom(u[i,0], u_int, fc, channel_slope, gravity_coeff))
        self.boundary_conditions(u, t)
        return u
