...
def source_mom_2(h, q, n, grav, mun, rho_mud, slope):
    # return (h-np.power((q/(h**2)),n))
    return slope*h*grav-mun/rho_mud*(((1.0+2.0*n)*(q/h))/(n*h))**n
...
class Solver1dNMDIM:
    ...
    def source_rk3(self, u, t):
        ...
        # q_int_1 = 0.0
        # in vectorized form
        q_int_1 = u[j0,1] + self.dt*(source_mom_2(u[j0,0], u[j0,1], self.n_coeff, self.grav, self.mun, self.rho_mud, self.slope))
        q_int_2 = (3.0/4.0)*u[j0,1] + (1.0/4.0)*q_int_1 + (1.0/4.0)*self.dt*(source_mom_2(u[j0,0], q_int_1, self.n_coeff, self.grav, self.mun, self.rho_mud, self.slope))
        u[j0,1] = (1.0/3.0)*u[j0,1] + (2.0/3.0)*q_int_2 + (2.0/3.0)*self.dt*(source_mom_2(u[j0,0], q_int_2, self.n_coeff, self.grav, self.mun, self.rho_mud, self.slope))
    ...
    def source_rk104(self, u, t):
        ...
        for i in range(1, 5+1):
            q1 = q1 + self.dt/6.0*(source_mom_2(u[j0,0], q1, self.n_coeff, self.grav, self.mun, self.rho_mud, self.slope))
    ...
    def source_crank_nicolson(self, u, t):
        ...
        for i in range(2, arr_length-1):
            ...
            q_explicit = u[i,1]+self.dt*source_mom_2(u[i,0], u[i,1], self.n_coeff, self.grav, self.mun, self.rho_mud, self.slope)
    ...
    def source_pr(self, u, t):
        ...
        for i in range(2, arr_length-1):
            k1 = optimize.fsolve(source_mom_imex_1, (source_mom_2(u[i,0], u[i,1], self.n_coeff, self.grav, self.mun, self.rho_mud, self.slope)), args=(u[i,0], u[i,1], self.dt, self.n_coeff, gamma_coeff))
            k2= optimize.fsolve(source_mom_imex_2, (source_mom_2(u[i,0], u[i,1], self.n_coeff, self.grav, self.mun, self.rho_mud, self.slope)), args=(u[i,0], u[i,1], k1, self.dt, self.n_coeff, (1-2.0*gamma_coeff), gamma_coeff))
